    ...
    <div class="w-full max-w-md space-y-6 p-4">
      ...
      <form class="space-y-4" @submit.prevent="handleSubmit">
        ...
        <div class="grid gap-2 relative">
          ...
          <Input id="password" v-model="password" :type="showPassword ? 'text' : 'password'" required :disabled="isLoading" />
          <Button type="button" variant="ghost" size="icon" class="absolute right-1 top-7 h-7 w-7" @click="showPassword = !showPassword">
            <component :is="showPassword ? EyeOff : Eye" class="h-4 w-4" />
          </Button>
        </div>
        <div class="grid gap-2 relative">
          <Label for="password_confirmation">Confirm New Password</Label>
          <Input id="password_confirmation" v-model="password_confirmation" :type="showPasswordConfirmation ? 'text' : 'password'" required :disabled="isLoading" />
          <Button type="button" variant="ghost" size="icon" class="absolute right-1 top-7 h-7 w-7" @click="showPasswordConfirmation = !showPasswordConfirmation">
